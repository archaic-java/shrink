...
final class SessionGoals {
  ...
  record Instrumentation(Goal analyze, Goal publish, Log log) {}

  static Instrumentation instrumentation() {
    var diagnostics = exactlyOne(Diagnostics.class);
    Log log = exactlyOne(Log.class);
    return new Instrumentation(diagnostics.goal("diagnostics.analyze", log), diagnostics.goal("diagnostics.publish", log), log);
  }
...
final class RunningSession implements AutoCloseable {
  ...
  RunningSession(CompilerAdapter compiler) throws IOException {
    ...
    server = Thread.ofVirtual().start(() -> {
      try { status.complete(new Session(serverInput, serverOutput, compiler, instrumentation.analyze(), instrumentation.publish(), instrumentation.log()).run()); }
