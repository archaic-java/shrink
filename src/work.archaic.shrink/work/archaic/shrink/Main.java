...
public final class Main {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      serve.run(() -> {
        if (new Session(System.in, System.out, compiler, analyze, diagnostics).run() != 0) {
      ...
      if (log != null) {
        try { log.write("shrink cannot continue: " + failure); }
        catch (IOException logFailure) { failure.addSuppressed(logFailure); }
      }
    }
    System.exit(status);
  }
